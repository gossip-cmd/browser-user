#!/usr/bin/env python3
"""
Facebook Comment Collector - 专门用于爬取 Facebook 评论
支持：评论爬取、回复提取
"""
...
class FacebookCollector(BaseCollector):
    ...
    def _get_comment_selector(self) -> str:
        """获取 Facebook 评论选择器"""
        return "[data-testid='UFICommentContainer'], [class*='UFI'], .comment"

    def _wait_for_comments(self):
        """等待 Facebook 评论加载完成"""
        print("[INFO] 等待 Facebook 评论加载...")
    ...
    def _get_comment_elements(self) -> List:
        """获取 Facebook 评论元素列表"""
        selectors = [
            "[data-testid='UFICommentContainer']",
            "[class*='UFIComment']",
            "[class*='fbreddit_comment']",
            ".comment",
        ]
    ...
    def _extract_comment(self, element) -> Optional[Dict]:
        ...
        try:
            ...
            author = ""
            for selector in [
                "a[class*='UFICommentUserName']",
                "[data-testid='UFICommentUserName']",
                "a[class*='name']",
                "a[href*='/profile']",
            ]:
                try:
            ...
            for selector in [
                ...
                try:
                    ...
                    if content and len(content) > 10:  # Facebook 评论通常较长
                        break
                except NoSuchElementException:
                    continue
            ...
            print(f"[WARN] 提取 Facebook 评论失败: {e}")
            return None
    ...
    def _extract_comment_replies(self, comment_element, max_replies: int = 100) -> List[Dict]:
        ...
        try:
            ...
            show_replies_selectors = [
                "[data-testid='UFICommentReplyButton']",
                "[class*='UFIReplyLink']",
                "a[class*='reply']",
            ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Facebook Comment Collector - 爬取 Facebook 评论",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 基本用法
  %(prog)s --url "https://facebook.com/xxx/posts/123456789"

  # 提取回复
  %(prog)s --url "https://facebook.com/xxx/posts/123456789" --ensure_fedback
        """
    )
    ...
    # 创建爬虫并收集评论
    try:
        with FacebookCollector(headless=args.headless) as collector:
