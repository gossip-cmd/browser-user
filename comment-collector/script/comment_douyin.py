#!/usr/bin/env python3
"""
Douyin Comment Collector - 专门用于爬取抖音评论
...
class DouyinCollector(BaseCollector):
    ...
    def _get_comment_selector(self) -> str:
        """获取抖音评论选择器"""
        return ".comment-item, [class*='comment'], .feed-item"
...
def main():
    parser = argparse.ArgumentParser(
        description="Douyin Comment Collector - 爬取抖音评论",
...
示例:
  ...
  # 使用 Cookie 登录
  %(prog)s --url "https://www.douyin.com/video/123456789" --cookies "name=value"

  # 提取回复
  %(prog)s --url "https://www.douyin.com/video/123456789" --ensure_fedback
        """
    )
    ...
    # 创建爬虫并收集评论
    try:
        with DouyinCollector(headless=args.headless) as collector:
